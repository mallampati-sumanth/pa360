...
import { useCheckCompleteness, useCreateRequest, usePARequirementCheck, usePatients, usePayers, usePlans, useProviders, useServices } from '@/features/authorization-workspace/hooks'
...
export default function NewAuthorizationPage() {
  ...
  const patientQuery = usePatients(); const providerQuery = useProviders(); const payerQuery = usePayers(); const planQuery = usePlans(); const serviceQuery = useServices()
  const patients = patientQuery.data?.results ?? patientQuery.data ?? []; const providers = providerQuery.data?.results ?? providerQuery.data ?? []; const payers = payerQuery.data?.results ?? payerQuery.data ?? []; const plans = planQuery.data?.results ?? planQuery.data ?? []; const services = serviceQuery.data?.results ?? serviceQuery.data ?? []
  ...
  async function identifyCase() {
    ...
    setCaseIds([created.authorization_id])
    setCaseId(created.authorization_id)
    setResult({
      is_required: requirementResult.is_required,
      needs_review: requirementResult.needs_review,
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    {step === 1 && <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]"><section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-semibold text-slate-900">Identify the case</h2><p className="mt-1 text-sm text-slate-500">Select a patient ID and one or more requested services.</p><div className="mt-6 grid gap-5 md:grid-cols-2"><Select label="Patient ID" value={form.patient} onChange={selectPatient} options={patients} id="patient_id" name="patient_id" placeholder="Select patient ID..." /><ReadOnlyField label="Patient name" value={patients.find((patient: Option) => patient.patient_id === form.patient)?.name || ''} placeholder="Auto-filled after patient selection" /><Select label="Ordering provider" value={form.provider} onChange={(v) => setField('provider', v)} options={providers} id="provider_id" name="provider_name" placeholder="Select provider..." /><Select label="Payer" value={form.payer} onChange={(v) => setField('payer', v)} options={payers} id="payer_id" name="payer_name" placeholder="Auto-filled from patient coverage" /><Select label="Plan" value={form.plan} onChange={(v) => setField('plan', v)} options={plans} id="plan_id" name="plan_name" placeholder="Auto-filled from patient coverage" /><MultiSelect label="Requested services" value={form.service_codes} onChange={(v) => setField('service_codes', v)} options={services} id="service_code" name="service_name" placeholder="Select one or more services..." /><Field label="Diagnosis" value={form.diagnosis} onChange={(v) => setField('diagnosis', v)} placeholder="ICD-10 or clinical diagnosis" /><div className="md:col-span-2"><Field label="Clinical indication" value={form.clinical_indication} onChange={(v) => setField('clinical_indication', v)} placeholder="Why are these services needed?" area /></div></div><button disabled={!ready || createRequest.isPending || checkRequirement.isPending} onClick={identifyCase} className="mt-6 inline-flex items-center gap-2 rounded-md bg-[#1356a1] px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50"><ShieldCheck className="h-4 w-4" />{createRequest.isPending ? 'Creating cases...' : 'Check authorization requirement'}</button></section><aside className="space-y-4"><InfoCard icon={Search} title="Rules first" text="Each selected service is evaluated with deterministic rules." /><InfoCard icon={Sparkles} title="AI stays assistive" text="AI can flag missing evidence, but a human verifies every submission." /></aside></div>}
    ...
    {step === 5 && <section className="rounded-xl border border-slate-200 bg-white p-10 text-center shadow-sm"><Check className="mx-auto h-12 w-12 rounded-full bg-emerald-100 p-3 text-emerald-600" /><h2 className="mt-4 text-2xl font-semibold text-slate-900">Human review started</h2><p className="mx-auto mt-2 max-w-lg text-sm text-slate-500">The authorization is ready for verification before payer submission.</p><Link href={`/specialist/authorizations/${caseId}`} className="mt-6 inline-flex rounded-md bg-[#1356a1] px-5 py-2.5 text-sm font-semibold text-white">Open review workspace</Link></section>}
  </div>
}
...
function MultiSelect({ label, value, onChange, options, id, name, placeholder }: { label: string; value: string[]; onChange: (v: string[]) => void; options: Option[]; id: string; name: string; placeholder: string }) {
  const toggleService = (serviceCode: string) => {
    onChange(value.includes(serviceCode) ? value.filter((item) => item !== serviceCode) : [...value, serviceCode])
  }
  ...
  return (
    <fieldset className="flex min-w-0 flex-col gap-2 text-sm font-medium text-slate-700">
      ...
      <div className="rounded-md border border-slate-300 bg-white p-2 focus-within:border-[#1769aa]">
        <div className="max-h-44 space-y-1 overflow-y-auto" aria-label={placeholder}>
          {options.map((option) => {
            ...
            return (
              <label key={option[id]} className={`flex cursor-pointer items-center gap-3 rounded-md px-3 py-2 transition-colors ${selected ? 'bg-blue-50 text-[#1356a1]' : 'hover:bg-slate-50'}`}>
                <input type="checkbox" checked={selected} onChange={() => toggleService(option[id])} className="h-4 w-4 accent-[#1356a1]" />
      ...
      <div className="flex min-h-6 flex-wrap gap-2">
        {value.length ? value.map((serviceCode) => {
          const service = options.find((option) => option[id] === serviceCode)
          return <span key={serviceCode} className="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-1 text-xs font-semibold text-[#1356a1]">{service?.[name] ?? serviceCode}</span>
        }) : <span className="text-xs font-normal text-slate-500">{placeholder}</span>}
      </div>
    </fieldset>
  )
}
function PatientSelect({ label, value, onChange, options, placeholder }: { label: string; value: string; onChange: (v: string) => void; options: Option[]; placeholder: string }) {
  const selectedPatient = options.find((p) => p.patient_id === value)
  return (
      ...
      <select value={value} onChange={(event) => onChange(event.target.value)} className="rounded-md border border-slate-300 bg-white px-3 py-2.5 outline-none focus:border-[#1769aa]">
        ...
        {options.map((option) => (
          <option key={option.patient_id} value={option.patient_id}>
            {option.patient_id} — {option.name}
          </option>
        ))}
      </select>
      {selectedPatient && (
        <span className="text-xs text-slate-500">Selected: <strong>{selectedPatient.name}</strong></span>
      )}
    </label>
  )
}
